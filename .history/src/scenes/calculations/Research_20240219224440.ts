...
export default class Research extends Scene {
  ...
  private _drawGrid() {
    ...
    const rowHeight = this.cameras.main.height / 64;
    const rowSpacing = this.cameras.main.height / 64;
    const subLevelSpacing = rowSpacing / 2;
    const startX = 0;
    let startY = this.cameras.main.height / 64;
    const totalRows = 6;
    const container = this.add.container(
      this.cameras.main.width / 2 - columnWidth - rowSpacing / 2,
      0
    );
