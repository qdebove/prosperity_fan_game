  ...
  create() {
    ...
    this.calculations.forEach((calculation, index) => {
      ...
      container.on("pointerdown", () => {
        if (
          this.scene.isActive<ScoreDisplayer<typeof calculation>>(
            "ScoreDisplayer"
          )
        ) {
          this.scene.stop("ScoreDisplayer");
        } else {
          this.scene.launch<ScoreDisplayer<typeof calculation>>(
            "ScoreDisplayer",
            {
              calculation,
              x: (index * container.width) / 2 + (index + 1) * 5,
